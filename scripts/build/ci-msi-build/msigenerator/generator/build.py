...
grafana_oss = {
    ...
    "directory_refs": ["GrafanaX64Dir"],
    "components": ["grafana.wxs", "grafana-service.wxs", "grafana-firewall.wxs"],
}
...
def build_msi(
    ...
    try:
        cmd = """
          ...
          -template fragment \
          -out {}""".strip().format(
            HEAT, target_dir_name, cgname, cgdir, outfile
        )
        print(cmd)
        os.system(cmd)
    ...
    os.chdir(src_dir)
    generate_firewall_wxs(
        env, PRODUCT_VERSION, "/tmp/scratch/grafana-firewall.wxs", target_dir_name
    )
    generate_service_wxs(
        env,
        PRODUCT_VERSION,
        "/tmp/scratch/grafana-service.wxs",
        target_dir_name,
        NSSM_VERSION,
    )
    generate_product_wxs(
        env, config, features, "/tmp/scratch/product.wxs", target_dir_name
    )
    ...
    try:
        ...
        cmd = (
            "{} -ext WixFirewallExtension -ext WixUtilExtension -v -arch x64 {}".format(
                CANDLE, filename
            )
        )
        print(cmd)
    ...
    msi_filename = "/tmp/scratch/grafana-{}{}.windows-amd64.msi".format(
        PRODUCT_VERSION, hash
    )

    if is_enterprise:
        msi_filename = "/tmp/scratch/grafana-enterprise-{}{}.windows-amd64.msi".format(
            PRODUCT_VERSION, hash
        )
    ...
    features = [
        {
            ...
            "title": PRODUCT_TITLE,
            "component_groups": [
                {"ref_id": "GrafanaX64", "directory": "GrafanaX64Dir"}
            ],
        },
        {
            "name": "GrafanaService",
            ...
            "component_groups": [
                {"ref_id": "GrafanaServiceX64", "directory": "GrafanaServiceX64Dir"}
            ],
        },
    ]
    build_msi(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        formatter_class=lambda prog: argparse.HelpFormatter(
            prog, max_help_position=90, width=110
        ),
        add_help=True,
    )
    parser.add_argument(
    ...
    if args.build:
        ...
        grafana_version, grafana_hash, is_enterprise = detect_version(
            DIST_LOCATION)
    ...
    if is_enterprise:
        if grafana_hash:
            zip_file = "{}/grafana-enterprise-{}-{}.windows-amd64.zip".format(
                DIST_LOCATION, grafana_version, grafana_hash
            )
            extracted_name = "grafana-{}-{}".format(
                grafana_version, grafana_hash)
        else:
            zip_file = "{}/grafana-enterprise-{}.windows-amd64.zip".format(
                DIST_LOCATION, grafana_version
            )
            extracted_name = "grafana-{}".format(grafana_version)
    else:
        # the file can have a build hash
        if grafana_hash:
            zip_file = "{}/grafana-{}-{}.windows-amd64.zip".format(
                DIST_LOCATION, grafana_version, grafana_hash
            )
            extracted_name = "grafana-{}-{}".format(
                grafana_version, grafana_hash)
