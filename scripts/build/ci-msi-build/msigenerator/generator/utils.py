...
def get_nssm(tmpPath, version):
    ...
    filename = wget.download(url, out=target_filename,
...
def get_zip(version, target_filename):
    ...
    if exists:
        return target_filename
    url = "https://s3-us-west-2.amazonaws.com/grafana-releases/release/grafana-{}.windows-amd64.zip".format(
        version
    )
...
def detect_version(dist_path):
    ...
    p1 = re.search(
        r"grafana-(enterprise-)?(\d\.\d\.\d)-(.+)\.windows-amd64\.zip$", firstFile
    )
    p2 = re.search(
        r"grafana-(enterprise-)?(\d\.\d\.\d)\.windows-amd64\.zip$", firstFile
    )
...
def generate_service_wxs(
    env, grafana_version, scratch_file, target_dir, nssm_version="2.24"
):
    template = env.get_template("common/grafana-service.wxs.j2")
    output = template.render(
        grafana_version=grafana_version, nssm_version=nssm_version)
    fh = open(scratch_file, "w")
    fh.write(output)
    fh.close()
    shutil.copy2(scratch_file, target_dir)
...
def generate_oracle_environment_wxs(
    env, instant_client_version, scratch_file, target_dir
):
